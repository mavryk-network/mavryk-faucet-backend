...
The Mavryk Faucet Backend (frontend code [here](https://github.com/mavryk-network/mavryk-faucet)) provides a reliable and secure way to distribute Mav to users. Through the implementation of a Proof of Work (PoW) mechanism, combined with CAPTCHA, we ensure users expend computational resources, thereby preventing bots and malicious actors from spamming and draining the faucet.

Here's a general flow of how it works:

1. **Requesting Mav**: A user initiates the process by making a request for a certain amount of Mav. The backend responds by sending a challenge to the user.
2. **Solving Challenges**: The user must solve the challenge by finding a correct solution. The complexity of the challenge can vary, and the number of challenges to be solved scales linearly with the amount of Mav requested.
3. **Verification & Receiving Mav**: Once the user submits a solution, the backend verifies it. If the solution is correct but there are more challenges to be solved, the user will be sent another challenge. This repeats until all challenges are solved correctly. Only then is the requested Mav granted to the user.

## Programmatic Faucet Usage

For programmatic usage of the faucet, we provide an npm package `@mavrykdynamics/get-mav`. The code can be found [here](https://github.com/mavryk-network/mavryk-faucet/tree/main/getMav). Please refer to it for more details on how to use it. This script can be run from a JavaScript program or directly from a shell. It interacts with the backend to request Mav, solve the required challenges, and verify the solutions.
...
- `MIN_CHALLENGES`: Minimum number of challenges required for the minimum Mav request (default: `1`)
- `MAX_CHALLENGES`: Maximum number of challenges required for the maximum Mav request (default: `550`)
- `MAX_CHALLENGES_WITH_CAPTCHA`: Maximum number of challenges required for the maximum Mav request when a captcha is used (default: `66`)

### Configuring Challenges

The `MAX_CHALLENGES`, `MIN_CHALLENGES`, `CHALLENGE_SIZE`, `DIFFICULTY`, `MIN_MAV`, `MAX_MAV`, and `MAX_CHALLENGES_WITH_CAPTCHA` environment variables control the number and difficulty of the challenges that a user must solve to receive Mav.
...
The `MAX_CHALLENGES` and `MIN_CHALLENGES` variables determine the maximum and minimum number of challenges that a user must solve to receive the max and min amount of Mav, respectively. The actual number of challenges scales linearly with the amount of Mav requested. The proportion of the requested Mav to the maximum Mav (`MAX_MAV` or `MAX_CHALLENGES_WITH_CAPTCHA` if captcha is used) is calculated, and the number of challenges is scaled based on this proportion.

For example, assume with a `DIFFICULTY` of 4 and `CHALLENGE_SIZE` of 2048 the average time to find a solution is approximately 1.09 seconds when using the `get-mav` script from the CLI or in a Node.js program, and approximately 4.6 seconds when solving challenges in the browser with the faucet frontend. Therefore, if `MAX_MAV` is set to 6000 and `MAX_CHALLENGES` is set to 550, it would take a user about 10 minutes (600 seconds) to receive 6000 Mav when using the `get-mav` script. With `MAX_CHALLENGES_WITH_CAPTCHA` set to 66, when using CAPTCHA via the browser it should take about 5 minutes to solve the challenges and receive Mav. The actual time may vary a bit depending on the user's computational resources.
...
For faucets hosted on https://testnets.mavryk.network, their backend urls can be found at https://testnets.mavryk.network/teztnets.json under `faucet_url`.

### **GET `/info`**

Returns details like the faucet's address, whether captcha and challenges are enabled, the maximum balance allowed, and the min and max Mav request amounts.
...
Initiate the Mav request procedure.
...
Submit challenge solutions and receive Mav.
...
- Grants the requested Mav amount and returns a transaction hash.

```json
{ "txHash": "oo7X..." }
```

**Note**: If `DISABLE_CHALLENGES` is `true`, `amount` should be sent in the request to the `/verify` endpoint, which will immediately grant Mav.
